...
# This dictionary will store the data from the CSVs so we only have to read the files once.
VEHICLE_DATA = {}
...
def load_vehicle_data():
    ...
    """
    global VEHICLE_DATA
    if 'brands' in VEHICLE_DATA:  # Check if data is already loaded
        return
    ...
    try:
        ...
        # --- Process Car Data ---
        # Clean up company names (e.g., "FERRARI" -> "Ferrari") and remove leading/trailing spaces.
        cars_df['Company Names'] = cars_df['Company Names'].str.strip().str.title()
        
        # Group models by their brand for quick and easy lookup later.
        ...
        # Set empty data to prevent repeated load attempts on error
...
class VehicleDataService:
    @staticmethod
    def get_brands():
        load_vehicle_data() # Ensure data is loaded
        return VEHICLE_DATA.get('brands', [])

    @staticmethod
    def get_models_for_brand(brand_name):
        load_vehicle_data() # Ensure data is loaded
        # Standardize the brand name for matching (e.g., "ford" -> "Ford")
